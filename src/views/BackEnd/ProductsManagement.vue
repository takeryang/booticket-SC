<template>
  <div class="products-management">
      ...
      <table class="table">
        <thead>
          <tr>
            <th width="10%">分類</th>
            <th width="35%">商品名稱</th>
            <th class="text-right" width="10%">售價</th>
            <th class="text-right" width="20%">商品狀態</th>
            <th class="text-center" width="15%">編輯</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item of filterProduct(searchVal)" :key="item.id">
            <td>{{ item.category }}</td>
            <td>{{ item.title }}</td>
            <td class="text-right">{{ item.price | currency}}</td>
            <td class="text-right">
              <span v-if="item.is_enabled" class="text-success">上架中</span>
    ...
    <!-- <Pagination :pagination="pagination" @changePage="getProducts(page)" /> -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      filterProduct(val) {
        let allProducts = Object.values(this.allProducts)

        return allProducts.filter(function (item) {
          return item.category.match(val) || item.title.toLowerCase().match(val)
        })
      },

      // filterProduct(val) {
      //   return this.products.filter(function (item) {
      //     return item.title.match(val) || item.category.match(val)
      //   })
      // },
    },
